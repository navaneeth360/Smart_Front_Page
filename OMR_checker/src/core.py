...
import src.qr_codes as qr_codes
...
class ImageInstanceOps:
    ...
    def apply_preprocessors(self, file_path, in_omr, template):
        ...
        cv2.imshow("Image of OMR after resize, first step", in_omr)
        cv2.waitKey(0)
    ...
    def read_omr_response(self, template, image, name, save_dir=None):
        print("Inside read_omr_response now...")
        ...
        try:
            ...
            print("Hello from line 211 in core.py")
            # Get mean bubbleValues n other stats
            all_q_vals, all_q_strip_arrs, all_q_std_vals = [], [], []
            total_q_strip_no = 0
            for field_block in template.field_blocks:
                if (field_block.field_type not in constants.CUSTOM_BLOCKS):
                    print("Field block loop entered in read_omr_response")
                    print("The field_block is : ", field_block.name)
                    print("Field block type is : ", field_block.field_type)
            ...
            for field_block in template.field_blocks:
                if (field_block.field_type not in constants.CUSTOM_BLOCKS):
                    ...
                    for field_block_bubbles in field_block.traverse_bubbles:
                        ...
                        block_q_strip_no += 1
                        total_q_strip_no += 1
                elif (field_block.field_type == "QR_CODE"):
                    print("QR block detected, calling function to read qr_code")
                    # Let's crop the image to only include the qr
                    y_start, x_start, length = field_block.origin[0], field_block.origin[1], field_block.dimensions[0]
                    cropped_image = qr_img[x_start-150:x_start+length+150, y_start-150:y_start+length+150]

                    # Let's try sharpening
                    # Sharpening kernel (simple 3x3)
                    ...
                    cv2.imshow("Cropped and Sharpened Image : ", sharpened_image)
                    cv2.waitKey(0)
                    cv2.destroyAllWindows()

                    print("The origin is : ", field_block.origin)

                    # TODO: Use the cropped image for QR so that multiple QRs can be there in the page
                    cv2.imshow("Cropped Image : ", cropped_image)
                    cv2.waitKey(0)
                    cv2.destroyAllWindows()

                    data2 = qr_codes.get_qr_data(qr_img)
                    print("Did we get any data from qr_img ?", data2)
                    data = qr_codes.get_qr_data(cropped_image)
                    print("Did we get any data from cropped_image ?", data)
                    data3 = qr_codes.get_qr_data(sharpened_image)
                    print("Did we get any data from sharpened_image ?", data3)
                    if (data is None):
                        data = field_block.empty_val
                    omr_response[field_block.parsed_field_labels[0]] = data
                    # /for field_block
                
                elif (field_block.field_type == "TEXT_BOX"):
                    print("core.py - read_omr_responses, text box detected !")
                    n_boxes = len(field_block.parsed_field_labels)
                    y_start, x_start, height, length = field_block.origin[0], field_block.origin[1], field_block.dimensions[0], field_block.dimensions[1] 
    
                    for i in range(n_boxes):
                        # cropped_img = text_img[x_start + int(length*0.1):x_start+int(length*0.9), y_start + int(height*0.1):y_start+int(height*0.9)]
                        cropped_img = text_img[x_start:x_start+length, y_start:y_start+height]
                        y_start += height

                        # cv2.imshow("Image of character", cropped_img)
                        # cv2.waitKey(0)
                        # cv2.destroyAllWindows()

                        outp = qr_codes.recognize_character(cropped_img)
                        if outp is None:
                            outp = " "
                        print("Character read is : ", outp)
                        omr_response[field_block.parsed_field_labels[i]] = outp
                
                elif (field_block.field_type == "NUM_BOX"):
                    print("core.py - read_omr_responses, number box detected !")
                    n_boxes = len(field_block.parsed_field_labels)
                    y_start, x_start, height, length = field_block.origin[0], field_block.origin[1], field_block.dimensions[0], field_block.dimensions[1] 
                    ...
                    for i in range(n_boxes):
                        ...
                        cv2.imshow("Image of number", cropped_img)
                        cv2.waitKey(0)
                        cv2.destroyAllWindows()

                        outp = qr_codes.recognize_number(cropped_img)
                        if outp is None:
                            outp = " "
                        print("Character read is : ", outp)
                        omr_response[field_block.parsed_field_labels[i]] = str(outp)

            

            print("Line 400 - what is omr_response looking like ?", omr_response)
    ...
    def draw_template_layout(img, template, shifted=True, draw_qvals=False, border=-1):
        print("draw template layout entered...")
        img = ImageUtils.resize_util(
            img, template.page_dimensions[0], template.page_dimensions[1]
        )
        print("Resize done !")
        print("Image dimensions after d_t_l are : ", img.shape)
        final_align = img.copy()
        for field_block in template.field_blocks:
            # print("Field block loop entered")
            # print("The field_block is : ", field_block.name)
            # print("Field block type is : ", field_block.field_type)
            s, d = field_block.origin, field_block.dimensions
            box_w, box_h = field_block.bubble_dimensions
            shift = field_block.shift
            print("Coordinates are : ", s, d)
            if shifted:
                cv2.rectangle(
                    final_align,
                    (s[0] + shift, s[1]),
            ...
            # For the below part, we need a separate condition for QR codes
            if (field_block.field_type not in constants.CUSTOM_BLOCKS):
                for field_block_bubbles in field_block.traverse_bubbles:
                    for pt in field_block_bubbles:
            ...
            elif (field_block.field_type == "TEXT_BOX" or field_block.field_type == "NUM_BOX"):
                print("core.py - text box detected, need to draw multiple boxes")
                # print(field_block.parsed_field_labels)
                n_boxes = len(field_block.parsed_field_labels) - 1
                s_temp = s.copy()
                for i in range(n_boxes):
                    s_temp[0] += d[0]
                    print("Box dimensions are - ", d[0], d[1])
                    if shifted:
                        cv2.rectangle(
                            final_align,
                            (s_temp[0] + shift, s_temp[1]),
                            (s_temp[0] + shift + d[0], s_temp[1] + d[1]),
                            constants.CLR_BLACK,
                            3,
                        )
                    else:
