...
class Template:
    ...
    def parse_and_add_field_block(self, block_name, field_block_object):
        print("Inside parse_and_add fb")
        field_block_object = self.pre_fill_field_block(field_block_object)
        print("No issue till instantiating FieldBlock")
...
class FieldBlock:
    ...
    def setup_field_block(self, field_block_object):
        # case mapping
        print("Setup field block entered")
        # Let's get the field_type to ensure that it's not a CUSTOM_BLOCK
        self.field_type = field_block_object.get("fieldType")
        print("Field type is : ", self.field_type)
        if (self.field_type not in constants.CUSTOM_BLOCKS):
            print("Is it correctly identifying a non-custom block")
            (
                bubble_dimensions,
                bubble_values,
                bubbles_gap,
            ...
            print("parsed field labels are ", self.parsed_field_labels)
            print(f"Field Block Labels: {self.name}", field_labels)
            print("Is this seen ?")
            self.origin = origin
            print("Is this seen ?")
        ...
        elif (self.field_type == "QR_CODE"):
             print("Is the QR block correctly identified ??")
             (
                bubble_dimensions,
                direction,
                field_labels,
                ...
                origin,
                height,
                length,             # Custom value needed for QR code dimension
                self.empty_val,
            ) = map(
                field_block_object.get,
                [
        ...
        elif (self.field_type == "TEXT_BOX" or self.field_type == "NUM_BOX"):
             print("Is the text box block correctly identified ??")
             (
                bubble_dimensions,
                direction,
                field_labels,
                field_type,
                origin,
                height,             # Custom values needed for text box dimension
                length,
                self.empty_val,
            ) = map(
                field_block_object.get,
        ...
        # Add additional conditions here for other non-OMR type custom blocks
    ...
    def calculate_block_dimensions(
        ...
        print("Field dimensions : ", fields_dimension, "Values dimensions : ", values_dimension)
        print("Parsed fields : ", len(self.parsed_field_labels), self.parsed_field_labels)
