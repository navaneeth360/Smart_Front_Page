...
def process_dir(
    ...
    # Update my template 
    my_template_path = curr_dir.joinpath(constants.MY_TEMPLATE_FILENAME)
    print("My template path is : ", my_template_path)
    my_template_exists = os.path.exists(my_template_path)
    if my_template_exists:
    ...
    if omr_files:
        ...
        print("Config summary done, going to process files or show template")
        if args["setLayout"]:
            print("setLayout arguement given, going to show layout now")
...
def show_template_layouts(omr_files, template, tuning_config):
    print("The template passed is : ", template)
    for file_path in omr_files:
        print("OMR loop entered")
        file_name = file_path.name
        file_path = str(file_path)
        in_omr = cv2.imread(file_path, cv2.IMREAD_GRAYSCALE)
        print("Initial image dimensions are : ", in_omr.shape)
        in_omr = template.image_instance_ops.apply_preprocessors(
            file_path, in_omr, template
        )
        print("Apply preprocessors done?")
        print("Preprocessed image dimensions are : ", in_omr.shape)
        template_layout = template.image_instance_ops.draw_template_layout(
            in_omr, template, shifted=False, border=2
        )
        print("template_layout created ?")
...
def process_files(
    ...
    for file_path in omr_files:
        ...
        # Just printing the read image
        cv2.imshow("Image of OMR before preprocessing", in_omr)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
        print("Template preprocessors are : ", template.pre_processors)
        print("Datatype of preprocessors is : ", type(template.pre_processors))

        if (my_template == None):
            my_template = template
            print("Didn't get my_template !! ")
        print("My Template preprocessors are : ", my_template.pre_processors)
        print("Datatype of preprocessors is : ", type(my_template.pre_processors))

        # my_in_omr = in_omr.copy()
        # my_in_omr = template.image_instance_ops.apply_preprocessors(
        #     file_path, my_in_omr, my_template
        # )
        # cv2.imshow("Image of OMR after crop page", my_in_omr)
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()

        # print("Done processing image for my_template !")
        ...
        print("Just before apply preprocessors !")
        in_omr = template.image_instance_ops.apply_preprocessors(
            file_path, in_omr, template
        )
        print("Apply preprocessors done !")
        ...
        print("Just before read_omr_response...")
        ...
        print("read_omr_response is done !")
        ...
        resp_array = []
        print("What are the output_columns ?", template.output_columns)
        for k in template.output_columns:
            resp_array.append(omr_response[k])
            print("What is the resp_array ? ", resp_array)
