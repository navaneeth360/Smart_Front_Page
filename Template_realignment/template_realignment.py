...
DEFAULT_SAVE_DIR   = os.path.join(SCRIPT_DIR, "final_templates")
DEFAULT_TEMPLATE_DIR  = r"C:\Users\Navaneeth\BTP_1\Final_repo\Template_maker\Templates\json"
...
def load_template_and_image():
    global template_images, template_texts, current_index, bg_tk_img, img_path

    # Load JSON
    # json_path = filedialog.askopenfilename(filetypes=[("JSON", "*.json")])
    json_path = filedialog.askopenfilename(
            initialdir=DEFAULT_TEMPLATE_DIR,
    ...
    img_mine = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)
    img = Image.open(img_path).convert('RGB')
    cv2.imshow("Image of OMR before preprocessing", img_mine)
    cv2.waitKey(0)
...
def save_omr_template(updated_images):
    ...
    out_path = filedialog.asksaveasfilename(
            ...
            title="Name final template file",
            defaultextension="_omr.json",
            filetypes=[("JSON", "*.json")]
        )
...
def save_new_template():
    global root, saved_template_result
    ...
    saved_template = save_omr_template(updated_images)

    if saved_template:
        saved_template_result = saved_template
        root.destroy()
...
# ---------------- GUI SETUP -----------------
img_path = ""
def init_gui(img_path_input):
    global root, canvas, info_label, img_path
    ...
    tk.Button(toolbar, text="Load Template", command=load_template_and_image).pack(side="left")
    ...
    return saved_template_result


# if __name__ == "__main__":
#     init_gui(r"c:\Users\Navaneeth\BTP_1\Final_repo\Template_realignment\cropped_page_pic.jpeg")
